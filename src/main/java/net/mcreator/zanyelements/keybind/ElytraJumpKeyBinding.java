...
package net.mcreator.zanyelements.keybind;
...
import net.mcreator.zanyelements.procedures.ElytraJumpOnKeyPressedProcedure;
...
public class ElytraJumpKeyBinding extends ZanyelementsModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public ElytraJumpKeyBinding(ZanyelementsModElements instance) {
		super(instance, 248);
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					ZanyelementsMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				ElytraJumpOnKeyPressedProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
